...
exports.DEFAULT_CUBE_DISPLAY_STATE = void 0;
exports.mountSmartCube = mountSmartCube;
const THREE = __importStar(require("three"));
const cube_appearance_1 = require("@/lib/cube-appearance");
const algorithms_1 = require("@/lib/algorithms");
exports.DEFAULT_CUBE_DISPLAY_STATE = {
    cameraDistance: 7,
    cameraLatitude: 35,
    cameraLongitude: 45,
};
...
const DEFAULT_CAMERA_DISTANCE = exports.DEFAULT_CUBE_DISPLAY_STATE.cameraDistance;
...
const DEFAULT_CAMERA_LATITUDE = exports.DEFAULT_CUBE_DISPLAY_STATE.cameraLatitude;
const DEFAULT_CAMERA_LONGITUDE = exports.DEFAULT_CUBE_DISPLAY_STATE.cameraLongitude;
...
const DEFAULT_GYRO_ORIENTATION = { top: "white", front: "green" };
const COLOR_VECTOR = {
...
function createHintArrowRing(material, geometries) {
...
function isInitialLowerLayerSticker(face, y) {
    if (face === "D")
        return true;
    return (face === "F" || face === "B" || face === "R" || face === "L") && y !== 1;
}
function makeCubie(x, y, z, colors, orientation, colorByCubeColor, facelets, formulaFacelets, showBackFaceProjection = false, backFaceProjectionDistance = DEFAULT_BACK_FACE_PROJECTION_DISTANCE, lowerLayerDimmed = false, size = 0.96) {
    const group = new THREE.Group();
    ...
    function createStickerMesh(sticker, stickerColor, stickerBaseFace, stickerLowerLayerDimmed, projected) {
    ...
    function createProjectionBorder(sticker) {
        const [px, py, pz] = sticker.pos;
        const localNormal = new THREE.Vector3(px, py, pz).normalize();
        const border = new THREE.LineSegments(new THREE.EdgesGeometry(stickerGeom), new THREE.LineBasicMaterial({
            color: 0x0e0e0c,
            transparent: true,
...
function isValidFormulaFacelets(facelets) {
...
function displayPositionToHardware(x, y, z, orientation) {
    const top = COLOR_VECTOR[orientation.top] ?? COLOR_VECTOR[cube_appearance_1.DEFAULT_ORIENTATION.top];
    const front = COLOR_VECTOR[orientation.front] ?? COLOR_VECTOR[cube_appearance_1.DEFAULT_ORIENTATION.front];
...
function selectLayer(cubies, layer) {
    const { axis, coord } = LAYER_AXIS[layer];
    return cubies.filter((cubie) => Math.round(cubie.userData.logicalPos[axis]) === coord);
}
function selectLayers(cubies, layers) {
    const first = layers[0];
    if (!first)
        return [];
    if (layers.length === 1)
        return selectLayer(cubies, first);
    const { axis } = LAYER_AXIS[first];
    const coords = new Set(layers.map((layer) => LAYER_AXIS[layer].coord));
    return cubies.filter((cubie) => coords.has(Math.round(cubie.userData.logicalPos[axis])));
}
function easeInOutCubic(t) {
    return t < 0.5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2;
}
function scaleMoveDuration(durationMs, backlog) {
    if (durationMs <= 0 || backlog <= 1)
        return durationMs;
    if (backlog >= 12)
        return Math.max(12, Math.round(durationMs * 0.06));
    if (backlog >= 8)
        return Math.max(18, Math.round(durationMs * 0.1));
    if (backlog >= 5)
        return Math.max(28, Math.round(durationMs * 0.18));
    if (backlog >= 3)
        return Math.max(45, Math.round(durationMs * 0.35));
...
function makeOrientationBasis(orientation) {
...
function mountSmartCube(container, options = {}) {
    const colors = {
    ...
    const displayOrientation = options.orientation ?? cube_appearance_1.DEFAULT_ORIENTATION;
    const displayFaceColors = (0, cube_appearance_1.getFaceColors)(displayOrientation);
    const colorByCubeColor = Object.fromEntries(Object.entries(displayFaceColors).map(([face, color]) => [color, colors[face]]));
    ...
    const defaultDisplayState = {
        cameraDistance: clamp(finiteOrDefault(options.defaultDisplayState?.cameraDistance, DEFAULT_CAMERA_DISTANCE), MIN_CAMERA_DISTANCE, MAX_CAMERA_DISTANCE),
        cameraLatitude: clamp(finiteOrDefault(options.defaultDisplayState?.cameraLatitude, DEFAULT_CAMERA_LATITUDE), -CAMERA_LATITUDE_LIMIT, CAMERA_LATITUDE_LIMIT),
        cameraLongitude: normalizeLongitude(finiteOrDefault(options.defaultDisplayState?.cameraLongitude, DEFAULT_CAMERA_LONGITUDE)),
    };
    ...
    function applyCameraOrbit() {
        cameraSpherical.set(cameraDistance * CAMERA_ORBIT_RADIUS_SCALE, (90 - cameraLatitude) * Math.PI / 180, cameraLongitude * Math.PI / 180);
    ...
    function applyCameraCoordinates(latitude, longitude) {
        ...
        const changed = Math.abs(nextLatitude - cameraLatitude) > 0.001 ||
    ...
    if (options.initialDisplayState) {
        cameraDistance = clamp(finiteOrDefault(options.initialDisplayState.cameraDistance, defaultDisplayState.cameraDistance), MIN_CAMERA_DISTANCE, MAX_CAMERA_DISTANCE);
        cameraLatitude = clamp(finiteOrDefault(options.initialDisplayState.cameraLatitude, defaultDisplayState.cameraLatitude), -CAMERA_LATITUDE_LIMIT, CAMERA_LATITUDE_LIMIT);
        cameraLongitude = normalizeLongitude(finiteOrDefault(options.initialDisplayState.cameraLongitude, defaultDisplayState.cameraLongitude));
    }
    applyCameraOrbit();
    ...
    viewRoot.position.set(finiteOrDefault(options.sceneOffset?.x, 0), finiteOrDefault(options.sceneOffset?.y, 0), finiteOrDefault(options.sceneOffset?.z, 0));
    scene.add(viewRoot);
    ...
    const displayFaceByHardwareColor = Object.fromEntries(Object.entries(displayFaceColors).map(([face, color]) => [color, face]));
    function colorForHardwareFace(face) {
        const hardwareColor = HARDWARE_FACE_COLOR[face];
        const displayFace = displayFaceByHardwareColor[hardwareColor];
        return displayFace ? colors[displayFace] : cube_appearance_1.COLOR_HEX[hardwareColor];
    }
    function registerProjectionObjects(cubie) {
        cubie.children.forEach((child) => {
            const userData = child.userData;
            if (!userData.projectionLocalNormal || (!userData.isProjectionSticker && !userData.isProjectionBorder))
                return;
            const object = child;
    ...
    function rebuildCubies(facelets) {
        ...
        for (let x = -1; x <= 1; x++) {
            for (let y = -1; y <= 1; y++) {
                for (let z = -1; z <= 1; z++) {
                    const cubie = makeCubie(x, y, z, colors, displayOrientation, colorByCubeColor, facelets, formulaFacelets, showBackFaceProjection, backFaceProjectionDistance, lowerLayerDimmed);
                    cubies.push(cubie);
    ...
    function applyBackFaceProjectionDistance(distance) {
    ...
    function paintSticker(mesh, color, baseFace) {
    ...
    function refreshStickerDisplayColors() {
        cubies.forEach((cubie) => {
            cubie.traverse((child) => {
                const sticker = child;
                if (!sticker.userData?.stickerLocalFace || !(child instanceof THREE.Mesh) || !(child.material instanceof THREE.MeshBasicMaterial))
                    return;
                child.material.color.set(lowerLayerDimmed && child.userData.stickerLowerLayerDimmed
                    ? LOWER_LAYER_DIMMED_STICKER
                    : child.userData.stickerBaseColor);
            });
        });
    }
    function repaintCubies(facelets) {
        ...
        cubies.forEach((cubie) => {
            ...
            cubie.traverse((child) => {
                ...
                if (!localFace || !(child instanceof THREE.Mesh) || !(child.material instanceof THREE.MeshBasicMaterial))
                    return;
                ...
                if (!rawFace)
                    return;
                paintSticker(child, colorForHardwareFace(rawFace), rawFace);
            });
        });
    ...
    function rotateLogicalPosition(pos, axis, physicalTurn, rotated) {
        if (physicalTurn === 2) {
            if (axis === "x")
                rotated.set(pos.x, -pos.y, -pos.z);
            else if (axis === "y")
                rotated.set(-pos.x, pos.y, -pos.z);
            else
                rotated.set(-pos.x, -pos.y, pos.z);
    ...
    function updateProjectionBorderVisibility() {
        if (!showBackFaceProjection || !projectionBorderVisibilityDirty)
            return;
    ...
    function buildQueuedMove(moves, durationMs) {
        const first = moves[0];
        if (!first)
            return null;
        const firstAxis = LAYER_AXIS[first.layer].axis;
        const layerTurns = new Map();
        ...
        const layers = [];
        let sharedTurn = null;
        let sharedTargetTurn = null;
        for (const [layer, rawTurn] of layerTurns) {
            const normalized = ((rawTurn % 4) + 4) % 4;
            if (normalized === 0)
                continue;
            const physicalTurn = (normalized === 3 ? -1 : normalized);
            const targetTurn = physicalTurn === 2 && rawTurn < 0 ? -2 : physicalTurn;
            if (sharedTurn !== null && physicalTurn !== sharedTurn)
                return null;
            if (sharedTargetTurn !== null && targetTurn !== sharedTargetTurn)
                return null;
            sharedTurn = physicalTurn;
            sharedTargetTurn = targetTurn;
            layers.push(layer);
        }
        if (sharedTurn === null || sharedTargetTurn === null || layers.length === 0)
            return null;
        return {
            layers,
            axis: firstAxis,
    ...
    function applyHintMove(move) {
        ...
        const queuedMove = buildQueuedMove((0, algorithms_1.expandMoveNotation)(move), 0);
        if (!queuedMove) {
            hintArrow.group.visible = false;
            return;
        }
        ...
        const hintDirection = queuedMove.targetAngle < 0 ? -1 : 1;
    ...
    function applyGyroQuaternion(quaternion, immediate) {
        if (interactionLocked)
            return;
        const hardwareOrientation = gyroHardwareOrientation.set(quaternion.x, quaternion.z, -quaternion.y, quaternion.w).normalize();
    ...
    function onPointerMove(event) {
        ...
        if (pointerPositions.size >= 2) {
            ...
            if (pinchDistance > 0 && nextPinchDistance > 0) {
                ...
                if (applyCameraDistance(nextDistance))
    ...
    function loop(now) {
        ...
        if (hintArrow.group.visible) {
            ...
            hintArrow.rings.forEach((ring) => {
                if (ring.visible)
                    ring.rotateZ(deltaSec * 0.9 * hintSpinSign);
            });
        }
    ...
    return {
        ...
        setFormulaFacelets(facelets) {
            if (!isValidFormulaFacelets(facelets))
                return false;
        ...
        setBackFaceProjectionDistance(distance) {
            if (applyBackFaceProjectionDistance(distance))
                requestRender();
        },
        setHintMove(move) {
            applyHintMove(move);
